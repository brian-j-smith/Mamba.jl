...
data = Dict{Symbol,Any}(
  :y => y,
  :X => mapslices( x-> x/sqrt(var(x)), X, 1), #scaled to have variance 1
  :n => n,
  :p => p
)
X = data[:X]
...
model = Model(
  y = Stochastic(1,
    (sigma, mu) -> MvNormal(mu, sqrt(sigma)),
    false
  ),
  beta = Stochastic(1,
    (p, mu_b, sigma_b, gamma) ->
      begin
        UnivariateDistribution[
          begin
            if gamma[i] == 1
              Normal(mu_b[i], sqrt(sigma_b))
            else
              Normal(0, 10^12)
            end
          end
  ...
  alpha = Stochastic(
    (mu_a, sigma_a) -> Normal(mu_a, sqrt(sigma_a))
  ),
  gamma = Stochastic(1,
    () -> Bernoulli(0.5)
  ),
  ...
  mu = Logical(1,
    (alpha, X, theta) -> alpha + X * theta,
    false
  ),
  mu_a = Stochastic(:(Normal(0, 10^4)),false),
  mu_b = Stochastic(1, :(Normal(0, 10^4)),false),
  sigma_a = Stochastic(:(InverseGamma(0.0001, 0.0001)),false),
  sigma_b = Stochastic(:(InverseGamma(0.0001, 0.0001)),false),
  sigma = Stochastic( :(InverseGamma(0.0001, 0.0001)),true)
)

## Gibbs samplers for alpha and beta

Gibbs_sigma = Sampler([:sigma],
  (sigma, mu, y) ->
    begin
      a = length(y) / 2.0 + shape(sigma.distr)
      b = sumabs2(y - mu) / 2.0 + scale(sigma.distr)
      rand(InverseGamma(a, b))
    end
)

Gibbs_alpha = Sampler([:alpha],
  (alpha, beta, sigma, X, gamma, y) ->
    begin
      agamma = [1; gamma]
      idx1 = find(agamma)
      idx0 = find(1 - agamma)
      prior_mean = [mean(alpha.distr); map(mean, beta.distr)]
      prior_invcov = diagm([var(alpha.distr); map(var, beta.distr)])
      aX = hcat(ones(length(y)),X)
      Sigma = inv((diagm(agamma) * aX' * aX * diagm(agamma) / sigma + diagm(agamma) * prior_invcov * diagm(agamma))[idx1,idx1])
      mu = Sigma * (diagm(agamma)*aX' * y / sigma + diagm(agamma) * prior_invcov * prior_mean)[idx1]
      proposal = [alpha; beta]
      proposal[idx1] = rand(MvNormal(mu, Sigma))
      proposal[idx0] = 0.0
      proposal[1]
    end
)
...
inits = Dict{Symbol,Any}[
  Dict{Symbol,Any}(:y => data[:y], :alpha => mean(y), :beta => inv(X'X+eye(p))X'y, :gamma => rand(0:1,data[:p]), :sigma => var(y), :X => data[:X], :mu_a => 0, :mu_b => zeros(15), :sigma_a => 1, :sigma_b => 1.0),
  Dict{Symbol,Any}(:y => data[:y], :alpha => 1, :beta => randn(p), :gamma => rand(0:1,data[:p]), :sigma => 1, :X => data[:X], :mu_a => 0, :mu_b => zeros(15), :sigma_a => 1, :sigma_b => 1.0),
  Dict{Symbol,Any}(:y => data[:y], :alpha => 17, :beta => [15; -15; -10; 5; -10; -5; -10; 10; 40; -5; 0; 0; 0; 20; 5], :gamma => rand(0:1,data[:p]), :sigma => 1, :X => data[:X], :mu_a => 0, :mu_b => zeros(15), :sigma_a=>1, :sigma_b => 1.0),
  Dict{Symbol,Any}(:y => data[:y], :alpha => 17, :beta => [15; -15; -10; 5; -10; -5; -10; 10; 40; -5; 0; 0; 0; 20; 5], :gamma => rand(0:1,data[:p]), :sigma => 1, :X => data[:X], :mu_a => 0, :mu_b => zeros(15), :sigma_a=>1, :sigma_b => 1.0)
]
...
## Sampling Scheme
scheme = [BMG([:gamma]), Gibbs_alpha, Gibbs_beta, Slice([:sigma], [10.0]), Slice([:mu_a, :sigma_a], [1.0, 1.0]),Slice([:mu_b, :sigma_b], [ones(15), 1.0])]
setsamplers!(model, scheme)

## MCMC Simulations
sim1 = mcmc(model, data, inits, 10000, burnin=1000, thin=1, chains=4)
describe(sim1)

## Discrete Gibbs Sampling

## Sampling Scheme
scheme = [DGS([:gamma]), Gibbs_alpha, Gibbs_beta, Slice([:sigma], [10.0]), Slice([:mu_a, :sigma_a], [1.0, 1.0]),Slice([:mu_b, :sigma_b], [ones(15), 1.0])]
setsamplers!(model, scheme)

## MCMC Simulations
sim2 = mcmc(model, data, inits, 10000, burnin=1000, thin=1, chains=4)
describe(sim2)

## Binary Modified Metropolised Gibbs Sampling

## Sampling Scheme
scheme = [BMMG([:gamma], p), Gibbs_alpha, Gibbs_beta, Slice([:sigma], [10.0]), Slice([:mu_a, :sigma_a], [1.0, 1.0]),Slice([:mu_b, :sigma_b], [ones(15), 1.0])]
setsamplers!(model, scheme)

## MCMC Simulations
sim3 = mcmc(model, data, inits, 10000, burnin=1000, thin=1, chains=4)
describe(sim3)

## Binary Hamiltonian Monte Carlo

## Sampling Scheme
scheme = [BHMC([:gamma]), Gibbs_alpha, Gibbs_beta, Slice([:sigma], [10.0]), Slice([:mu_a, :sigma_a], [1.0, 1.0]),Slice([:mu_b, :sigma_b], [ones(15), 1.0])]
setsamplers!(model, scheme)

## MCMC Simulations 
sim4 = mcmc(model, data, inits, 10000, burnin=1000, thin=1, chains=4)
describe(sim4)
